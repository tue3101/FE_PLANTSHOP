export function useCartAnimation() {
  const createFlyAnimation = (
    event: MouseEvent | Event,
    productImage: HTMLElement | null = null
  ): void => {
    // Tìm button
    ...
    if (
      productImage &&
      productImage instanceof HTMLElement &&
      typeof productImage.getBoundingClientRect === "function"
    ) {
      imageElement = productImage
    } else {
      // Tự tìm hình ảnh trong product card (từ ProductList)
      const productCard =
        button.closest(".product-card") || button.closest('[class*="product"]')
      if (productCard) {
        ...
        imageElement =
          button.closest(".product-detail-image")?.querySelector("img") ||
    ...
    if (!cartIcon) {
      ...
      const fallbackCartIcon =
        (document.querySelector("[data-cart-icon]") as HTMLElement) ||
        (document.querySelector(".cart-icon") as HTMLElement) ||
        (document.querySelector('[aria-label*="cart" i]') as HTMLElement)
      if (!fallbackCartIcon) {
