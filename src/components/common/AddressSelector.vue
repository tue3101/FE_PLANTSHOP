<template>
    <div class="space-y-4">
        ...
        <div>
            <div class="flex gap-2 items-center flex-wrap mb-2">
                ...
                <p v-if="showShippingNotice" class="text-sm font-normal text-red-500">
                    Hiện tại cửa hàng chỉ hỗ trợ ship trong khu vực thành phố Hồ Chí Minh và tỉnh Bà Rịa - Vũng Tàu để
                    đảm bảo chất lượng sản
                    phẩm đến tay khách hàng.
                </p>
            </div>

            <select :id="`city-${_uid}`" v-model="cityId" :required="required" @change="handleCityChange" :class="[
        ...
        <div>
            ...
            <select :id="`district-${_uid}`" v-model="selectedDistrictId" :required="required" :disabled="!cityId"
                @change="handleDistrictChange" :class="[
                    'w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2',
                    focusRingClass,
        ...
        <div>
            ...
            <textarea :id="`address-detail-${_uid}`" v-model="addressDetail" :required="required" rows="3"
...
import { CITIES, DISTRICTS_BY_CITY, getCityById, getDistrictsByCityId, getDistrictById } from '@/constants/locations'
