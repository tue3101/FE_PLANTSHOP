  ...
  >
    ...
    <div class="mt-4 mb-4 space-y-2">
      <div class="flex flex-col">
        <div>
          ...
          <p class="text-sm text-gray-600 mt-1">Ngày đặt: {{ formatDate(order.order_date) }}</p>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between">
        <span class="text-sm text-gray-600"
          >Phương thức thanh toán ({{ getPaymentMethodName(order) }}):</span
        >
        <span
          :class="[
            'inline-block px-3 py-1 rounded-full text-sm font-semibold',
    ...
    <div class="mb-4 pt-3 border-t border-gray-300">
      ...
      <div class="space-y-2">
        ...
        >
          <img
            :src="getProductImage(detail.product)"
            :alt="getProductName(detail.product)"
    ...
    <div class="mt-4 pt-4 flex justify-end gap-3">
      ...
      <button
        ...
        :disabled="order.status !== 'PENDING_CONFIRMATION'"
        :class="[
      ...
      <router-link
        v-if="order.shipping_status === 'DELIVERED' && !hasAllProductsReviewed"
...
const loadFullOrderInfo = async () => {
  ...
  console.log(
    "⚠️ Note: API /api/orders/{id} does not return shipping info, trying to load from user info instead"
  )
...
const shippingFee = computed(() => {
  ...
  if (
    ...
    props.order.shipping_fee !== undefined &&
    props.order.shipping_fee !== 0
  ) {
...
const loadPaymentInfo = async () => {
  ...
  console.log(
    "⚠️ Order does not have payment info, loading from API for order:",
    props.order.order_id
  )
  try {
