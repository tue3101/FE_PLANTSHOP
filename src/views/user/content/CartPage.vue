<template>
  <div class="min-h-screen bg-gray-50 py-8">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="bg-white rounded-lg shadow p-4 flex items-center justify-between">
            ...
            <span class="text-sm text-gray-600"
              >{{ selectedCount }} / {{ selectableItemsCount }} sản phẩm được chọn</span
            >
          </div>
          ...
          <div
            ...
            :class="['bg-white rounded-lg shadow p-6 flex flex-col sm:flex-row gap-4 relative']"
          >
            ...
            <div class="flex-1 flex flex-col justify-between">
              ...
              <div class="flex items-end mt-4">
                <div
                  class="flex-1"
                  :class="{
                    'pointer-events-none opacity-70': isOutOfStock(item) || isDeleted(item),
                  }"
                >
                  <QuantitySelector
                    :model-value="item.quantity"
                    @update:model-value="(value) => updateItemQuantityAndSelected(item, value)"
                    @exceed-max="(value) => handleExceedMaxInCart(item, value)"
                    :min="1"
                    :max="getMaxQuantity(item)"
                    :disabled="isOutOfStock(item) || isDeleted(item)"
                    label="Số lượng mua:"
                    :show-error="true"
                  />
                </div>
    ...
    <DeleteModal
      :show-modal="showStockModal"
      mode="info"
      title="Số lượng không đủ"
      :message="stockModalMessage"
      @close="handleCloseStockModal"
      @update:show-modal="showStockModal = $event"
    />
  </div>
</template>
...
const isAllSelected = computed(() => {
  const selectableItems = cartStore.cartItems.filter(
    (item) => !isOutOfStock(item) && !isDeleted(item)
  )
    ...
    try {
      ...
      const updatePromises = cartStore.cartItems
        ...
        .map((item) => {
          const identifier = item.cart_detail_id
          return cartStore
            .updateQuantity(identifier, item.quantity, shouldSelect)
            .catch((error) => {
...
watch(
  ...
  (newItems, oldItems) => {
    ...
    const currentIdentifiers = new Set(newItems.map((item) => item.cart_detail_id))
