          ...
          <div class="mb-6">
            ...
            <p class="text-gray-700 mb-1">
              <span v-if="isDepositPayment">
                Đơn hàng #{{ orderId }} đã được đặt cọc thành công. Đơn hàng sẽ được xử lý trong
                thời gian sớm nhất.
              </span>
              <span v-else> Đơn hàng #{{ orderId }} của bạn đã được xác nhận </span>
            </p>
            <p class="text-sm text-gray-500">
              Bạn sẽ được chuyển về trang đơn hàng trong giây lát...
            </p>
          </div>
          ...
          <div class="mb-6">
            ...
            <p class="text-gray-700 mb-4">
              {{
                errorMessage ||
                (isDepositPayment
                  ? "Vui lòng thử lại đặt cọc hoặc liên hệ với cửa hàng"
...
onMounted(async () => {
  ...
  if (isDepositPayment.value) {
    ...
    console.log(
      "🔔 Deposit payment detected. DEPOSIT ID:",
      orderIdParam,
      "Actual order ID:",
      actualOrderId
    )
  ...
  let resultCode = resultCodeParam
    ? typeof resultCodeParam === "string"
      ? parseInt(resultCodeParam)
      : resultCodeParam
    : null
  ...
  if (resultCode === 0 || resultCode === "0") {
    ...
    if (orderId.value) {
      ...
      try {
        console.log(
          `🔄 [BẮT BUỘC] Đang hủy đơn hàng do ${paymentType} thất bại (CANCELLED):`,
          orderId.value
        )
        ...
        if (cancelResponse?.data?.success) {
          ...
          console.warn(
            "⚠️ Hủy đơn hàng không thành công, thử lại với updateOrderStatusStore:",
            cancelResponse?.data
          )
      ...
      if (orderCancelled) {
        ...
        console.error(
          "❌ CẢNH BÁO: Không thể hủy đơn hàng sau nhiều lần thử. Đơn hàng có thể vẫn còn tồn tại."
        )
      }
    } else {
