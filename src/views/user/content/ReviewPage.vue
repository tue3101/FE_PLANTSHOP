<template>
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="container mx-auto px-4 max-w-6xl">
      ...
      <!-- Empty State -->
      <div
        v-if="!isLoading && !errorMessage && productsToReview.length === 0"
      ...
      <div v-if="!isLoading && !errorMessage && productsToReview.length > 0" class="space-y-6">
        <div
          v-for="productItem in productsToReview"
        ...
        >
          <div class="flex flex-col md:flex-row gap-4 mb-4">
            <!-- Product Image -->
            <img
              :src="getProductImage(productItem.product)"
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-gray-600 mb-1">Đơn hàng: #{{ productItem.order_id }}</p>
              <p class="text-sm text-gray-600">Số lượng: {{ productItem.quantity }}</p>
            </div>
          </div>
          ...
          <div v-if="!productItem.hasReview" class="border-t pt-4">
            ...
            <div class="mb-4">
              <label
                :for="`comment-${productItem.product_id}`"
                class="block text-sm font-medium text-gray-700 mb-2"
              >
                Nhận xét (tùy chọn):
              </label>
              <textarea
                :id="`comment-${productItem.product_id}`"
                v-model="productItem.comment"
                rows="3"
                placeholder="Chia sẻ cảm nhận của bạn về sản phẩm này..."
                class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500"
                :disabled="productItem.isSubmitting"
              ></textarea>
            </div>
            ...
            <div class="flex justify-end mt-4">
              <button
                type="button"
                @click="
                  productItem.isEditing
                    ? handleUpdateReview(productItem)
                    : handleCreateReview(productItem)
                "
          ...
          <div v-else class="border-t pt-4">
            <div class="flex items-center justify-between">
              <div class="flex-1">
                ...
                <StarRating
                  :model-value="productItem.existingReview?.rating || 0"
                  :disabled="true"
                />
              ...
              <div class="flex gap-2 ml-4">
                <button
                  type="button"
                  @click="handleEditReview(productItem)"
                ...
                <button
                  type="button"
                  @click="handleDeleteReview(productItem)"
