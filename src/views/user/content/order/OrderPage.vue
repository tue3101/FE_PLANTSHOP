  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="container mx-auto px-4 max-w-6xl">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <router-link
          to="/order-history"
          class="text-green-700 hover:text-green-500 transition-colors"
        >
          <History :size="28" />
        </router-link>
      </div>
      ...
      <!-- Empty State -->
      <div
        v-if="!isLoading && !errorMessage && displayedOrdersCount === 0"
    ...
    <div v-if="showCancelModal" class="fixed inset-0 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <p class="mb-6 text-gray-700">
          ...
          <span class="font-bold">#{{ selectedOrderForCancel?.order_id }}</span> không?
        </p>
...
const ensureOrderStatus = async (orderId, desiredStatus) => {
  try {
    ...
    console.error(
      `❌ OrderPage - Lỗi khi cập nhật trạng thái đơn hàng ${orderId} -> ${desiredStatus}:`,
      error
    )
  }
}
...
const handleMoMoPaymentReturn = async () => {
  ...
  const isDepositPayment =
    depositOrderId !== null ||
    (momoOrderId && typeof momoOrderId === "string" && momoOrderId.startsWith("DEPOSIT_"))
  ...
  try {
    ...
    if (
      ...
      console.log(
        `💰 OrderPage - Kiểm tra deposit.paid cho đơn hàng #${orderIdNum} (TRƯỚC khi xử lý payment status)...`
      )
      try {
        const depositResponse = await depositStore.getDepositByOrderIdStore(orderIdNum)
        const depositData =
          depositResponse?.data || depositResponse?.success ? depositResponse.data : depositResponse
        ...
        if (depositInfo) {
          ...
          if (!isPaid) {
            console.log(
              `⚠️ OrderPage - Deposit paid = 0 (hoặc false), hủy đơn hàng ngay lập tức...`,
              {
                depositPaid,
                orderId: orderIdNum,
              }
            ...
            try {
              ...
              console.error(
                `❌ OrderPage - Lỗi khi hủy đơn hàng #${orderIdNum} do deposit paid = 0:`,
                cancelError
              )
              // Thử lại một lần nữa
              try {
                ...
                orderCancelled = true
                console.log(
                  `✅ OrderPage - Đã hủy đơn hàng #${orderIdNum} do deposit paid = 0 (retry)`
                )
              } catch (retryError) {
                console.error(
                  `❌ OrderPage - Không thể hủy đơn hàng #${orderIdNum} sau nhiều lần thử:`,
                  retryError
                )
              }
            }
            ...
            console.log(
              `✅ OrderPage - Deposit đã được thanh toán (paid = ${depositPaid}) cho đơn hàng #${orderIdNum}`
            )
          }
        } else {
          console.log(
            `⚠️ OrderPage - Không tìm thấy deposit info cho đơn hàng #${orderIdNum}, có thể deposit chưa được tạo`
          )
        }
      } catch (depositError) {
        console.error(
          `❌ OrderPage - Lỗi khi lấy deposit info cho đơn hàng #${orderIdNum}:`,
          depositError
        )
    ...
    if (paymentStatus === "FAILED") {
      ...
      try {
        ...
        console.error(
          `❌ OrderPage - Lỗi khi hủy đơn hàng do ${paymentType} thất bại:`,
          cancelError
        )
        // Thử lại một lần nữa
        try {
    ...
    if (paymentStatus === "PROCESSING" || !paymentStatus) {
      console.log(
        `⏳ OrderPage - ${paymentType} đang PROCESSING hoặc chưa có status, kiểm tra order...`
      )
          ...
          if (
            ...
            try {
              ...
              try {
                ...
                console.error(
                  `❌ OrderPage - Không thể hủy đơn hàng sau nhiều lần thử:`,
                  retryError
                )
              }
            }
      ...
      if (timeDiff > tenMinutes) {
        ...
        console.log(
          `⚠️ OrderPage - Đã quá 10 phút, ${paymentType} chưa có kết quả, hủy đơn hàng để an toàn`
        )
        ...
        try {
          ...
          console.error(
            `❌ OrderPage - Lỗi khi hủy đơn hàng do ${paymentType} quá thời gian:`,
            cancelError
          )
        }
  ...
  for (const order of ordersToCheck) {
    ...
    try {
      ...
      if (depositInfo) {
        const depositPaid = depositInfo.paid
        const isPaid =
          depositPaid === true || depositPaid === 1 || depositPaid === "1" || depositPaid === "true"

        console.log(
          `💰 OrderPage - Đơn hàng #${orderId} - deposit paid:`,
          depositPaid,
          "isPaid:",
          isPaid
        )
        ...
        if (!isPaid) {
          ...
          try {
            ...
            try {
              ...
              console.error(
                `❌ OrderPage - Lỗi khi cập nhật payment status cho đơn hàng #${orderId}:`,
                paymentError
              )
            }
          } catch (cancelError) {
      ...
      if (depositError.response?.status !== 404) {
        console.error(
          `❌ OrderPage - Lỗi khi lấy deposit info cho đơn hàng #${orderId}:`,
          depositError
        )
      }
    }
      ...
      try {
        ...
        try {
          ...
          if (orderResponse.data.success && orderResponse.data.data) {
            ...
            if (paymentId) {
              console.log(
                "Found payment_id from order object, updating status to FAILED:",
                paymentId
              )
              await paymentStore.updatePaymentStatusStore(paymentId, "FAILED")
              console.log("Payment status updated to FAILED successfully")
