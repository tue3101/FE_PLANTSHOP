<template>
  <div class="p-8">
    ...
    <LoadingErrorState
      ...
      loadingMessage="Đang tải đánh giá..."
    ...
    <div v-if="!isLoading && !errorMessage" class="overflow-x-auto">
      ...
      <DataPager
        v-model="currentPage"
        :items="filteredActiveReviews"
        :items2="filteredDeletedReviews"
        :page-size="PAGE_SIZE"
        :show-filter="true"
        :show-active="true"
        :show-status-filter="true"
        :status-options="ratingOptions"
        v-model:selected-status="ratingFilter"
        v-model:selected-active="selectedActive"
        controls-class="mb-1"
      >
        <template #default="{ items }">
          <CommonTable
            :headers="['ID', 'NGƯỜI DÙNG', 'SẢN PHẨM', 'ĐÁNH GIÁ', 'NHẬN XÉT', 'NGÀY ĐÁNH GIÁ']"
            :keys="['review_id', 'user', 'product', 'rating', 'comment', 'created_at']"
            :data="items"
            row-key="review_id"
          ...
          >
            ...
            <template #cell-rating="{ item }">
              <div class="flex items-center gap-1">
                <span
                  v-for="star in 5"
                  :key="star"
                  :class="['text-lg', star <= item.rating ? 'text-yellow-400' : 'text-gray-300']"
                  >★</span
                >
            ...
            <template #actions="{ item }">
              <div :class="['[&>div>button:nth-child(2)]:hidden']">
                <ButtonCommon
...
const refreshReviewsData = async () => {
  await reviewStore.getAllReviewsStore()

  try {
    await reviewStore.getAllDeletedReviewsStore()
  } catch (error) {
